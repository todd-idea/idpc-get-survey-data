...
/**
 * The Main class provides a way to test the REST API by retrieving the complete report model (report, report model,
...
public class Main {
	...
	private static final def DEFAULT_SURVEY_ID = 1
	...
	private static def surveyID = DEFAULT_SURVEY_ID
	...
	public static void main(String[] args) {
		...
		def cli = new CliBuilder( usage: 'Main -v -s -st -h host -p port -b basePath -sid surveyID -t type -a "TestClient" -k "ABCDEFG123456"' )
		cli.with {
		...
		def reportModel = getCompleteReportModel(surveyID, type)
		def endTime = new Date().time
		stats["Runtime"] = endTime - startTime

		println "Report models for survey ${surveyID} and type ${type} loaded."
